...
Reconciliation never rewrites the configuration files you own, and it rebuilds the files Blue owns from scratch each time. See [Managed configuration](/0.2.0/concepts/managed-configuration) for which files fall on each side, what a governed launch inherits from yours, and the full list of merge rules.
