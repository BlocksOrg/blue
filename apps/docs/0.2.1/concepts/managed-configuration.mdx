...
Paths shown as `runtime/…` are relative to Blue's managed data directory, which is `~/.config/blue` on macOS and Linux and `%LOCALAPPDATA%\Blue\Data` on Windows. See [Windows paths and migration](/0.2.1/cli/windows-paths) for the full Windows layout.
...
For the per-harness overlay strategy see [Supported harnesses](/0.2.1/cli/harnesses), for how a policy revision reaches your machine see [Configuration lifecycle](/0.2.1/concepts/configuration), and for the complete policy shape see [Governance configuration](/0.2.1/reference/governance-config).
