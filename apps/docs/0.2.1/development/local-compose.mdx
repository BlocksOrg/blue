...
This workflow is for repository maintainers and contributors. Follow the [CLI install steps](/0.2.1/cli/commands#install) if your organization already operates Blue. Operators designing a deployment should start with the platform-neutral [Deployment contract](/0.2.1/deployment/runtime-contract); the Compose defaults are not a production baseline.
