...
Keep `BETTER_AUTH_SECRET` stable after the initial deployment. Follow [Configure blue.yaml](/0.2.1/deployment/blue-yaml) for the full environment mapping, authentication contract, storage endpoints, managed repositories, and gateway secrets.
...
Use [Kubernetes with Helm](/0.2.1/deployment/production) for a concrete implementation, or translate this contract into your platform's service, task, job, and load-balancer resources.
...
See [Bring your own gateway](/0.2.1/concepts/gateway-mode), [Gateway access](/0.2.1/admin/gateway-access), and [Custom gateway provisioners](/0.2.1/admin/custom-gateway-provisioners) for the complete contract.
