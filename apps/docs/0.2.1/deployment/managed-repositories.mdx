...
<Tabs>
  ...
  <Tab title="GitHub Enterprise">
    ...
    Add an exact hostname to `download_hosts` only when the configured server redirects archive downloads to that trusted hostname. Wildcards, ports, paths, and embedded credentials are rejected. Same-host downloads require no entry.
  </Tab>
...
| `download_hosts` | Optional | Additional exact trusted cross-host archive redirect destinations. HTTP archive redirects are HTTPS-only and capped at five hops, and credentials are sent only to the original scheme, host, and port. Bitbucket Cloud's Git acquisition path refuses redirects entirely. |
...
See [Managed packages](/0.2.1/admin/managed-packages) for capability mapping and client installation behavior, and [Production deployment](/0.2.1/deployment/production) for storage and secret-management requirements.
