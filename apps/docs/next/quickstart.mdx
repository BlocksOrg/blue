...
Bare `blue` launches your preferred agent. Hold Ctrl and press `]` inside a session for status, health, and reload controls. See [CLI commands](/next/cli/commands) for installation options and the full command list, including `blue status` and `blue verify` for scripts.
...
Adding one top-level `gateway` section turns on gateway mode. The release bundle ships a starter with every section filled in. See [Configure blue.yaml](/next/deployment/blue-yaml) for every field, and [Governance configuration](/next/reference/governance-config) for the full policy schema.
...
Two public HTTPS origins are exposed: the dashboard and the Control API. Gateway mode adds a third for the inference proxy. The [Deployment contract](/next/deployment/runtime-contract) lists the image commands, ports, health checks, required environment variables, and the external dependencies each mode needs.
...
<Columns cols={2}>
  <Card title="Configure blue.yaml" icon="file-code" href="/next/deployment/blue-yaml">
  ...
  <Card title="Bring your own gateway" icon="route" href="/next/concepts/gateway-mode">
