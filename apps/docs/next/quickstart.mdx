...
Gateway mode adds the inference proxy, an organization-operated LiteLLM gateway, credential provisioning and encryption, private resolver traffic, and per-user pseudotokens. It does not change the core dashboard, Control API, worker, PostgreSQL, or object-storage contract.
