...
[session capture](/next/admin/session-capture): enabling either feature does
...
Blue stores the gateway credential envelope-encrypted and gives the client an
opaque pseudotoken. Provider credentials, LiteLLM administrator credentials,
and the managed gateway key remain server-side.
...
[custom executable provisioner](/next/admin/custom-gateway-provisioners) can
...
See [Bring your own gateway](/next/concepts/gateway-mode) for deployment topology and
[Blue YAML](/next/deployment/blue-yaml) for the complete configuration schema.
...
encrypted value, invalidate the old pseudotoken mapping, and issue a new opaque
pseudotoken. If ensure fails, Blue records the public error, stores no partial
...
strings, pseudotokens, virtual keys, or arbitrary headers in request history.
...
<Warning>
  ...
  virtual key to a developer machine. Only the pseudotoken and client-reachable
  proxy URL belong in personalized harness configuration.
</Warning>
