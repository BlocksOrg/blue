...
Remote gateway-key cleanup is best effort. Local pseudotoken resolution is removed transactionally, so governed clients lose gateway access even if the upstream gateway is unavailable.
