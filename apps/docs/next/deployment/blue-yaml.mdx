...
| `provisioner.model_catalog_refresh_seconds` | Model discovery interval in seconds; defaults to `300` and must be at least `30`. `HARNESS_GATEWAY_MODEL_CATALOG_REFRESH_SECONDS` overrides it. |
...
| `contract_version` | Wire contract version. Current clients support version 4. |
