...
`gateway.type`; it does not infer a gateway from the upstream URL. In Helm, set
`blue.gatewayType`, which is required with `blue.enableInferenceProxy=true` and
is injected into both the Control API and proxy workloads.

The LiteLLM administrator key remains a runtime secret supplied as `HARNESS_LITELLM_ADMIN_KEY`; do not put it in governance policy. See [Bring your own gateway](/next/concepts/gateway-mode) for the request path, pseudotoken model, and operational checks.
...
| `contract_version` | Wire contract version. Current clients support version 2. |
