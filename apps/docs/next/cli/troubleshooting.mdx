...
This revokes the stored session and starts a new device authorization flow. It does not delete configuration overlays or captured sessions. `blue logout && blue login` does the same thing in two steps.

`blue login` on its own already asks the service whether the stored session still works, so it re-authorizes without `--force` whenever the service rejects it. Use `--force` to switch accounts, or to replace a session that is still valid.
