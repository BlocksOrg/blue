...
| `blue login [--force]` | Verify the stored session with the service and, when it is missing or rejected, complete OAuth device authorization. `--force` revokes the stored session and re-authorizes even when it is still valid. |
