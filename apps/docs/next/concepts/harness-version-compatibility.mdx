...
Claude Code `2.1.242` is a compatibility boundary for gateway model controls.
Releases from `2.0.12` through `2.1.241` use `claude-v2_0_12` and retain
selected-model-only behavior. Releases from `2.1.242` use
`claude-v2_1_242`, which replaces Claude's built-in picker choices with Blue's
ordered assigned catalog and enforces the first assignment as the Default-row
fallback when no explicit managed model is selected.
