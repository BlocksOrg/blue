...
falls back to another installer if ownership is unverified or an installer fails.

| Installation | Ownership evidence | Repair |
| --- | --- | --- |
| npm global (all four harnesses, Unix) | `<prefix>/bin/<agent>` resolves to the expected package under `<prefix>/lib/node_modules`; package name and `bin` metadata match, and read-only npm global root/prefix probes confirm that prefix | `npm install -g --prefix <owned-prefix> <package>@<exact-version>` |
...
| Homebrew Claude/Codex/OpenCode | Resolved target under the matching `Caskroom` or `Cellar` package, including custom prefixes | Refuse automatic arbitrary-version pinning; use manual vendor/Homebrew guidance |
...
| Kimi legacy uv tool | Target belongs to a Python environment with `uv-receipt.toml` identifying `kimi-cli` and `pyvenv.cfg` | Refuse: legacy Python versions are not the current npm release line |
...
| Codex standalone/release download, custom Kimi/OpenCode roots, pnpm/bun wrappers, Windows wrappers/native layouts, other unverified copies | No supported ownership proof | Refuse automatic replacement; manually install a policy-compatible release at the active path |
...
unsupported for automatic repair until equivalent ownership checks are available.
...
Daemon runs, redirected/non-interactive commands, and `blue apply --yes` never install native CLIs. They fail closed and print ownership-aware manual guidance for an operator.
