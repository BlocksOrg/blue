...
Paths shown as `runtime/…` are relative to Blue's managed data directory, which is `~/.config/blue` on macOS and Linux and `%LOCALAPPDATA%\Blue\Data` on Windows. See [Windows paths and migration](/next/cli/windows-paths) for the full Windows layout.
...
For the per-harness overlay strategy see [Supported harnesses](/next/cli/harnesses), for how a policy revision reaches your machine see [Configuration lifecycle](/next/concepts/configuration), and for the complete policy shape see [Governance configuration](/next/reference/governance-config).
