...
Use `blue login --force` to re-authorize while a session is still valid — switching accounts, or replacing a session you no longer trust. It revokes the stored refresh token before starting the new flow, so no grant is left live server-side.
