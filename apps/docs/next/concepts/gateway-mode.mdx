...
The Helm chart can issue this pair for you: with
`blue.internalTransport.certManager.enabled`, it declares the `Issuer` and
`Certificate` resources and cert-manager generates and renews both certificates,
so no Secret has to be made by hand.
