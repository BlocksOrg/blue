...
| Documentation | `http://127.0.0.1:3001` | Local Mintlify documentation site |
| Website | `http://127.0.0.1:3002` | Public website preview |
...
HARNESS_DOCS_URL=http://127.0.0.1:3001
```

The dashboard's **Documentation** sidebar link opens the Compose-hosted docs service by default. The `mintlify-cache` named volume retains the downloaded local-preview runtime between container recreations.
...
Named volumes retain PostgreSQL state, MinIO data, and the Mintlify preview cache. Adding `--volumes` deletes local state and should be used only when a full reset is intended.
