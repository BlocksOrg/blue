...
blue.yaml gateway.type
  ...
  → personalized GatewayConfig { type, proxy_url, pseudotoken, auth_style }
...
The data plane exchanges that pseudotoken for an upstream credential:

```text
agent request with pseudotoken
...
| `HarnessImplementation` | Version-specific pseudotoken placement and agent wire protocol | Gateway-type dispatch or upstream-provider behavior |
...
`GatewayRoute` contains only the Blue inference-proxy URL and pseudotoken. The
...
<Steps>
  ...
  <Step title="Extend policy only when necessary">
    Reuse `type`, `proxy_url`, `pseudotoken`, and `auth_style` when possible.
...
- Correct client route normalization without exposing a pseudotoken in debug
  output.
...
- Pseudotokens may reach clients. Upstream credentials and administrator secrets
  may not.
