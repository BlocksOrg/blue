...
This workflow is for repository maintainers and contributors. Follow the [Quickstart workstation steps](/0.1.0/quickstart#install-the-workstation-cli) if your organization already operates Blue. Operators designing a deployment should start with the platform-neutral [Deployment contract](/0.1.0/deployment/runtime-contract); the Compose defaults are not a production baseline.
