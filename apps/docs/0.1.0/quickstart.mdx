...
<Steps>
  ...
  <Step title="Choose how to run the services">
    ...
    <Columns cols={3}>
      <Card title="Kubernetes and Helm" icon="boxes-stacked" href="/0.1.0/deployment/production">
        Use the maintained chart and optional AWS OpenTofu starter as a reference production implementation.
      </Card>
      <Card title="Your platform" icon="server" href="/0.1.0/deployment/runtime-contract">
        Map the image commands, ports, health checks, configuration, and dependencies onto ECS, Nomad, VMs, or another scheduler.
      </Card>
      <Card title="Local evaluation" icon="docker" href="/0.1.0/development/local-compose">
  ...
  <Step title="Provide the common deployment contract">
    ...
    Follow the [Deployment contract](/0.1.0/deployment/runtime-contract) for the complete process, port, health-check, networking, and lifecycle requirements.
  </Step>
  ...
  <Step title="Configure policy, identity, and storage">
    ...
    Use [Configure blue.yaml](/0.1.0/deployment/blue-yaml) for the full schema, value precedence, and secret-reference rules. The mounted file is a deployment baseline; administrators can publish later policy revisions from the dashboard.
  </Step>
  ...
  <Step title="Sign in as the bootstrap administrator">
    ...
    Invite another user from **Members**, or continue with the bootstrap administrator for the first workstation test. See [Manage users and invitations](/0.1.0/admin/user-management) or [Identity provisioning](/0.1.0/admin/identity-provisioning) for managed OIDC and SCIM.
  </Step>
...
Follow [Bring your own gateway](/0.1.0/concepts/gateway-mode) for the routing model, [Gateway access](/0.1.0/admin/gateway-access) for operator configuration, and the [Deployment contract](/0.1.0/deployment/runtime-contract#optional-inference-proxy) for the additional workload requirements.

## Next steps

<Columns cols={2}>
  <Card title="Deployment contract" icon="list-check" href="/0.1.0/deployment/runtime-contract">
    Translate Blue's workload and service requirements to your platform.
  </Card>
  <Card title="Kubernetes reference" icon="boxes-stacked" href="/0.1.0/deployment/production">
    Use the maintained Helm chart and AWS infrastructure starter.
  </Card>
  <Card title="Managed extensions" icon="package" href="/0.1.0/admin/managed-packages">
    Publish digest-pinned skills, plugins, hooks, helpers, and MCP servers.
  </Card>
  <Card title="Troubleshooting" icon="wrench" href="/0.1.0/cli/troubleshooting">
