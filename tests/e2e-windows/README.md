...
Windows SDK. Run the suite manually on a Windows machine or an EC2 Windows
Full/Base VM. It requires no Docker, backend credentials, npm registry access or
inference calls at test runtime.
...
output on failure. This standalone Cargo workspace is deliberately separate from
`tests/e2e` and `tests/e2e-slim`; their Linux workflows and deployment image
